 ...
 * because it allows us to interleave calls from the custom malloc
 ...
 *  fit reasonably into both dense and sparse.
 *
 ...
 */
#define MAX_DENSE_HEAP (100 * (1UL << 20)) /* 100 MB */
...
static unsigned char
    *mem_brk_chunk; /* ditto, rounded up to a whole allocation chunk */
...
void setUBCheck(bool val) { checkUB = val; }
#endif
...
#ifdef USE_MSAN
const char *__msan_default_options(void) { return "abort_on_error=true"; }
#endif
...
void mem_init(bool do_sparse) {
  ...
  /* The sparse heap is used for internal bookkeeping and is not
     exposed to student code.  The dense heap is used directly by
     student code.  We manage a pseudo-break within the dense heap
     by mapping it PROT_NONE initially and then changing pages to
     PROT_READ|PROT_WRITE upon calls to mem_sbrk.  */
  int prot = sparse ? PROT_READ | PROT_WRITE : PROT_NONE;
  void *addr = mmap(start,                       /* suggested start*/
                    ...
                    0);                          /* offset */
  if (addr == MAP_FAILED) {
    fprintf(stderr, "FAILURE.  mmap couldn't allocate space for heap (%s)\n",
            strerror(errno));
    exit(1);
  }
  if (round_address_down(addr, mem_pagesize()) != addr) {
    fprintf(stderr, "FAILURE.  Initial heap address (%p) is not page aligned\n",
            addr);
    exit(1);
  }
  if (sparse) {
...
void mem_reset_brk(void) {
  ...
  if (sparse) {
    ...
    /* In order to make subsequent calls to mem_sbrk cost
       approximately what they did on the first pass, overwrite
       the entire heap with a fresh PROT_NONE mapping.  */
    if (mmap(heap, MAX_DENSE_HEAP, PROT_NONE,
             MAP_FIXED | MAP_PRIVATE | MAP_ANONYMOUS, -1, 0) == MAP_FAILED) {
      fprintf(stderr, "FAILURE.  deallocation of heap failed (%s)\n",
              strerror(errno));
      exit(1);
    }
#ifdef USE_MSAN
...
void *mem_sbrk(intptr_t incr) {
  ...
  unsigned char *new_brk_chunk = round_address_up(new_brk, mem_pagesize());
  if (!sparse) {
    ...
    if (new_brk_chunk > mem_brk_chunk &&
      ...
      fprintf(stderr, "ERROR: making %zd bytes at %p accessible failed (%s)\n",
              new_brk_chunk - mem_brk_chunk, (void *)mem_brk_chunk,
              strerror(errno));
      return (void *)-1;
    }
#ifdef USE_ASAN
    /* Tell ASan the precise location of the break.  */
    __asan_unpoison_memory_region(old_brk, (size_t)incr);
    if (new_brk < new_brk_chunk) {
      __asan_poison_memory_region(new_brk, (size_t)(new_brk_chunk - new_brk));
    }
#endif
#ifdef USE_MSAN
    /* Mark the requested section of the heap as uninitialized.  */
    __msan_allocated_memory(mem_brk, (size_t)incr);
#endif
  }
 ...
 */
void *mem_heap_lo(void) { return (void *)heap; }
 ...
 */
size_t mem_heapsize(void) { return (size_t)(mem_brk - heap); }

/*
 * mem_pagesize - returns the page size of the system
 */
size_t mem_pagesize(void) {
  static size_t pagesize = (size_t)-1;
  if (pagesize == (size_t)-1) {
    pagesize = (size_t)sysconf(_SC_PAGESIZE);
    // code in mem_init and mem_sbrk assumes the page size is a
    // power of two; check this (will also crash if sysconf fails)
...
static size_t page_id(const void *addr) {
  ptrdiff_t offset = (unsigned char *)addr - (unsigned char *)SPARSE_HEAP_START;
  assert(offset >= 0);
